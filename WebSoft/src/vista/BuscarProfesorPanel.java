 ...
 * @author ferchs
 */
public class BuscarProfesorPanel extends javax.swing.JPanel  {

    /**
     * Creates new form AgregarEstudiantePanel
     */
    protected ProfesoresPanel panelProfesores;
    public BuscarProfesorPanel(ProfesoresPanel panelProfesores) {
        initComponents();
    ...
    private void buscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buscarActionPerformed
        // TODO add your handling code here:
        String itemBusqueda=barraBusqueda.getText().toLowerCase();
        DefaultTableModel modelo=controlBuscarProfesorPanel.obtenerModeloBusqueda(itemBusqueda);
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (tabla.getModel().getColumnClass(columna).equals(Object.class)) {
                    ...
                    if(btn.getActionCommand().equals("visualizar")){
                        visualizarProfesor(id);
                    }
                    if(btn.getActionCommand().equals("editar")){
                        editarProfesor(id);
                    }
                    if(btn.getActionCommand().equals("eliminar")){
                        int respuesta=JOptionPane.showConfirmDialog(null, "¿Eliminar este docente?", "Confirmar Eliminacion", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                        if(respuesta==0){
                            eliminarProfesor(id,fila); 
                        }
                    }
                }
            }
    ...
    public void visualizarProfesor(String idProfesor){
        try {
        HashMap<String, String> campos=controlBuscarProfesorPanel.obtenerDatosProfesor(idProfesor);
        VisualizarProfesorPanel vep= new VisualizarProfesorPanel(this);
        vep.llenarCampos(campos);
        panelProfesores.mostarPanel(vep);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    
    public void editarProfesor(String idProfesor){
        try {
        HashMap<String, String> campos=controlBuscarProfesorPanel.obtenerDatosProfesor(idProfesor);
        EditarProfesorPanel epp= new EditarProfesorPanel();
        epp.llenarCampos(campos);
        panelProfesores.mostarPanel(epp);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    
    public void eliminarProfesor(String idProfesor,int fila){
        boolean eliminado=controlBuscarProfesorPanel.eliminarProfesor(idProfesor);
        if(eliminado){
    ...
    protected void volver(JPanel panel){
        panelProfesores.mostarPanel(panel);
    }
